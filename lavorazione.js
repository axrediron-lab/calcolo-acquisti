(function () {
  ...
  function accessState() { byId("workWorkspace").hidden=!key; if(!key)window.AppAuth.redirect(false); }
  ...
  function compactProcessingLabel(value){return {pending:"In sospeso",manual:"Quantità manuale",applying:"Invio da completare",automatic:"Completato"}[value]||"";}
  function render() {
    var completed=work.items.filter(item=>item.processing&&!["pending","applying"].includes(item.processing.quantity_status)).length;
    byId("workHeading").innerHTML='<div class="work-document-head"><div><p class="eyebrow">Documento Ready</p><h2>'+esc(work.document.document_number)+' · '+esc(displayDate(work.document.document_date))+'</h2></div><div class="work-document-summary"><span>'+work.items.length+' articoli</span><span>'+completed+' completati</span><span>'+(work.items.length-completed)+' da gestire</span></div></div><p>Il costo iniziale non viene mediato; la media parte dagli acquisti successivi.</p>';
    var rows=work.items.map((item,index)=>{
      ...
      var stateLabel=blocked?'Bloccato':item.processing?compactProcessingLabel(item.processing.quantity_status):'Da lavorare';
      var buyboxUrl='buybox.html?listing='+encodeURIComponent(item.listing_id)+'&order='+encodeURIComponent(documentKey);
      var buyboxLink=item.processing?'<a class="secondary-button compact-action" href="'+esc(buyboxUrl)+'">Prezzi e BuyBox →</a>':'<span class="work-muted">Dopo la conferma</span>';
