(function(){
  ...
  function marketRowHtml(listing,market,competitor,view){
    ...
    return '<tr class="'+statusClass+'" data-market-row="'+escapeHtml(market)+'">'+
      ...
      '<td><label class="price-box"><span class="price-entry-line"><input aria-label="Prezzo target ('+escapeHtml(currency)+')" data-price-field="target" data-listing-id="'+escapeHtml(listing.id)+'" data-market="'+escapeHtml(market)+'" inputmode="decimal" placeholder="0,00" value="'+escapeHtml(draft.target)+'"><span class="currency-suffix">'+escapeHtml(currency)+'</span></span><span class="price-result-slot" data-price-result="target">'+compactMarginHtml(targetEconomics,listing.id)+'</span></label></td>'+
