(function () {
  ...
  var displayDate = value => { var match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(value || "")); return match ? match[3] + "/" + match[2] + "/" + match[1] : String(value || ""); };
  ...
  function renderPreview() {
    byId("purchasePreview").innerHTML = docs.map((d, index) => {
      ...
      return '<details class="purchases-panel" open><summary>Documento ' + esc(d.number) + ' · ' + esc(displayDate(d.date)) + ' · ' + d.units + ' pezzi · ' + esc(money(d.total_cents)) + '</summary><p>' + references(d.references) + '</p><p class="document-state">' + esc(status) + '</p>' + linesTable(d.lines, d.status === "new") + '<button type="button" data-confirm="' + index + '" ' + (!d.token ? 'disabled' : '') + '>Salva documento nell’archivio</button><small>Non aggiorna le quantità su Back Market.</small></details>';
    }).join('');
  }
  ...
  async function history(append) {
    ...
    if (mode === "mappings") {
      byId("purchaseHistory").innerHTML = '<div class="purchases-table-wrap mapping-table-wrap"><table class="mapping-table"><thead><tr><th>Prodotto Ready</th><th>Inserzione Back Market</th><th>Ultima modifica</th><th>Azioni</th></tr></thead><tbody>' + historyRows.map(m => '<tr><td><strong class="mapping-ready-code">' + esc(m.ready_code) + '</strong><small>' + esc(m.ready_description || "Descrizione non disponibile") + '</small></td><td><span class="mapping-state ' + (m.listing_id ? 'active' : 'removed') + '">' + (m.listing_id ? 'Attivo' : 'Rimosso') + '</span><strong class="mapping-sku">' + esc(m.sku || "Da abbinare alla prossima importazione") + '</strong></td><td><span class="mapping-date">' + esc(new Date(m.updated_at).toLocaleString("it-IT")) + '</span></td><td><div class="mapping-actions"><button type="button" data-map="' + esc(m.ready_code) + '">' + (m.listing_id ? 'Modifica' : 'Abbina') + '</button>' + (m.listing_id ? '<button type="button" data-remove="' + esc(m.ready_code) + '" class="secondary danger-button">Rimuovi</button>' : '') + '<button type="button" data-audit="' + esc(m.ready_code) + '" class="secondary">Storico</button></div></td></tr>').join('') + '</tbody></table></div>';
    } else {
      byId("purchaseHistory").innerHTML = '<div class="purchases-table-wrap"><table class="order-table"><thead><tr><th>Ordine</th><th>Data</th><th>Righe</th><th>Pezzi</th><th>Costo totale EUR</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>' + historyRows.map(d => { var complete=d.processed_items>=d.item_count&&!d.pending_items,state=complete?'Completato':d.processed_items?'In lavorazione':'Da lavorare',quantityOnly=d.document_type==="quantity_only",kind=d.document_subtype==="ready_return"?'Carico resi':quantityOnly?'Solo quantità':'Acquisto'; return '<tr><td><span class="order-kind '+(quantityOnly?'quantity':'purchase')+'">'+esc(kind)+'</span><strong>' + esc(d.document_number) + '</strong><small>'+esc(d.document_label||'Ordine Ready')+'</small></td><td>' + esc(displayDate(d.document_date)) + '</td><td>' + d.row_count + '</td><td>' + d.units + '</td><td>' + (quantityOnly?'—':esc(money(d.total_cents))) + '</td><td><span class="order-state '+(complete?'done':'pending')+'">' + esc(state) + '</span></td><td><a class="table-action" href="lavorazione.html?key=' + encodeURIComponent(d.document_key) + '">'+(complete?'Rivedi':d.document_subtype==="ready_return"?'Carica':'Lavora')+'</a><button type="button" data-document="' + esc(d.document_key) + '">Dettagli</button></td></tr>'; }).join('') + '</tbody></table></div>';
    }
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (target.dataset.document) run(async () => {
      ...
      byId("documentTitle").textContent = (d.document_type==="quantity_only"?"Ripristino quantità ":"Documento ") + d.document_number + " · " + d.document_date;
      if(d.document_type==="quantity_only"){var isReturn=d.document_subtype==="ready_return",referencesLabel=isReturn?'Documenti Ready':'Orderline sorgenti';byId("documentContent").innerHTML='<p>'+esc(d.document_label)+' · '+d.units+' pezzi. Nessun prezzo o costo associato.</p><div class="purchases-table-wrap"><table><thead><tr><th>SKU</th><th>Prodotto</th><th>Quantità</th><th>'+referencesLabel+'</th></tr></thead><tbody>'+d.lines.map(l=>'<tr><td>'+esc(l.sku_snapshot)+'</td><td>'+esc(l.description)+'</td><td>'+esc(l.incoming_quantity)+'</td><td>'+esc((l.source_references||l.source_orderline_ids||[]).join(" · "))+'</td></tr>').join('')+'</tbody></table></div>';}
      else byId("documentContent").innerHTML = '<p>' + references(d.references) + '</p><p>' + esc(d.source.name) + ' · ' + d.units + ' pezzi · ' + esc(money(d.total_cents)) + '</p><p>Abbinamenti storici al momento del salvataggio. Quantità non inviate a Back Market.</p>' + linesTable(d.lines, false);
