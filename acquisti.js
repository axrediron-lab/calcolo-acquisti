(function () {
  ...
  async function history(append) {
    ...
    if (mode === "mappings") {
      ...
      byId("purchaseHistory").innerHTML = '<div class="purchases-table-wrap"><table class="order-table"><thead><tr><th>Ordine</th><th>Data</th><th>Righe</th><th>Pezzi</th><th>Costo totale EUR</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>' + historyRows.map(d => { var complete=d.processed_items>=d.item_count&&!d.pending_items,state=complete?'Completato':d.processed_items?'In lavorazione':'Da lavorare',quantityOnly=d.document_type==="quantity_only"; return '<tr><td><span class="order-kind '+(quantityOnly?'quantity':'purchase')+'">'+esc(quantityOnly?'Solo quantità':'Acquisto')+'</span><strong>' + esc(d.document_number) + '</strong><small>'+esc(d.document_label||'Ordine Ready')+'</small></td><td>' + esc(displayDate(d.document_date)) + '</td><td>' + d.row_count + '</td><td>' + d.units + '</td><td>' + (quantityOnly?'—':esc(money(d.total_cents))) + '</td><td><span class="order-state '+(complete?'done':'pending')+'">' + esc(state) + '</span></td><td><a class="table-action" href="lavorazione.html?key=' + encodeURIComponent(d.document_key) + '">'+(complete?'Rivedi':'Lavora')+'</a><button type="button" data-document="' + esc(d.document_key) + '">Dettagli</button></td></tr>'; }).join('') + '</tbody></table></div>';
    }
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (target.dataset.document) run(async () => {
      ...
      if(d.document_type==="quantity_only")byId("documentContent").innerHTML='<p>Ripristino annullamenti cliente · '+d.units+' pezzi. Nessun prezzo o costo associato.</p><div class="purchases-table-wrap"><table><thead><tr><th>SKU</th><th>Prodotto</th><th>Quantità</th><th>Orderline sorgenti</th></tr></thead><tbody>'+d.lines.map(l=>'<tr><td>'+esc(l.sku_snapshot)+'</td><td>'+esc(l.description)+'</td><td>'+esc(l.incoming_quantity)+'</td><td>'+esc(l.source_orderline_ids.join(" · "))+'</td></tr>').join('')+'</tbody></table></div>';
