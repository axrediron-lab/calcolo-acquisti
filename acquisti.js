(function () {
  ...
  async function history(append) {
    ...
    if (mode === "mappings") {
      ...
      byId("purchaseHistory").innerHTML = '<div class="purchases-table-wrap"><table class="order-table"><thead><tr><th>Documento Ready</th><th>Data</th><th>Righe</th><th>Pezzi</th><th>Costo totale EUR</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>' + historyRows.map(d => { var complete=d.processed_items>=d.item_count&&!d.pending_items,state=complete?'Completato':d.processed_items?'In lavorazione':'Da lavorare'; return '<tr><td><strong>' + esc(d.document_number) + '</strong></td><td>' + esc(displayDate(d.document_date)) + '</td><td>' + d.row_count + '</td><td>' + d.units + '</td><td>' + esc(money(d.total_cents)) + '</td><td><span class="order-state '+(complete?'done':'pending')+'">' + esc(state) + '</span></td><td><a class="table-action" href="lavorazione.html?key=' + encodeURIComponent(d.document_key) + '">'+(complete?'Rivedi':'Lavora')+'</a><button type="button" data-document="' + esc(d.document_key) + '">Dettagli</button></td></tr>'; }).join('') + '</tbody></table></div>';
    }
  }
