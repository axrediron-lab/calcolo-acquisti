(function () {
  ...
  async function history(append) {
    ...
    if (mode === "mappings") {
      ...
      byId("purchaseHistory").innerHTML = '<div class="purchases-table-wrap"><table><thead><tr><th>Documento Ready</th><th>Data</th><th>Righe</th><th>Pezzi</th><th>Costo totale EUR</th><th>Stato</th><th></th></tr></thead><tbody>' + historyRows.map(d => '<tr><td>' + esc(d.document_number) + '</td><td>' + esc(d.document_date) + '</td><td>' + d.row_count + '</td><td>' + d.units + '</td><td>' + esc(money(d.total_cents)) + '</td><td>Salvato · quantità non inviate</td><td><button type="button" data-document="' + esc(d.document_key) + '">Apri</button></td></tr>').join('') + '</tbody></table></div>';
    }
  }
